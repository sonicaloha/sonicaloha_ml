...
import numpy as np
import torch
from .models import build_ACT_model, build_CNNMLP_model, build_TactileACT_model, build_SonicACT_model
...
def get_args_parser():
    ...
    parser.add_argument('--num_selected', action='store', type=int, help='the action selected for temporal agg', required=False)
    parser.add_argument('--num_rollouts', action='store', type=int, help='num_rollouts', required=False)
...
def build_TactileACT_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
    ...
    model = build_TactileACT_model(args)
    model.cuda()
...
def build_SonicACT_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
    ...
    model = build_SonicACT_model(args)
    model.cuda()
